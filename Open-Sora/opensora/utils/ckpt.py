...
# Make tensornvme optional - it's only needed for async checkpoint saving
# If not available, we'll fall back to synchronous saving
try:
    from tensornvme.async_file_io import AsyncFileWriter
except ImportError:
    # If tensornvme is not installed, use a dummy type for type hints
    # The code will work fine without it, just without async I/O benefits
    AsyncFileWriter = None  # type: ignore
...
class CheckpointIO:
    def __init__(self, n_write_entries: int = 32):
        ...
        # AsyncFileWriter is optional - if tensornvme is not installed, these will be None
        # and the code will use synchronous saving instead
        self.writer: Optional[Any] = None  # type: ignore
        ...
        self.master_writer: Optional[Any] = None  # type: ignore
