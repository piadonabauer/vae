...
def setup_device() -> tuple[torch.device, DistCoordinator]:
    ...
    if not dist.is_initialized():
        # If not, check if we should initialize for single-GPU training
        # This happens when running with plain `python` instead of `torchrun`
        if os.environ.get("RANK") is None:
            # Initialize single-GPU distributed training
