...
# Deterministic dataloader: give each worker a distinct but reproducible seed (same base `seed`
# for every worker caused identical RNG streams across workers).
...
def prepare_dataloader(
    ...
    **kwargs,
    ...
    elif isinstance(dataset, PTVideoDataset):
        # PTVideoDataset has fixed-size videos, so we can use a regular sampler
        # Similar to TextDataset but with video data
        if process_group is None:
            return (
                DataLoader(
                    dataset,
...
def collate_fn_default(batch):
    ...
    use_mask = False
    if isinstance(batch[0], dict) and "mask" in batch[0] and isinstance(batch[0]["mask"], int):
