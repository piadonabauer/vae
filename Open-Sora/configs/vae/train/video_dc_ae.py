...
eval_every = 100  # Compute metrics on current batch every 100 steps (0 = disabled)

# Full evaluation pass (more expensive, every N steps)
full_eval_every = 500  # Run full evaluation every 500 steps (0 = disabled)
eval_num_samples = 32  # Number of samples for periodic full eval
eval_batch_size = 4  # Batch size for evaluation (defaults to training batch_size)
eval_use_ema = True  # Use EMA model for evaluation if available

# Final evaluation
final_eval = True  # Run final evaluation after training (default: True)
final_eval_num_samples = 100  # Number of samples for final evaluation