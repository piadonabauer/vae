...
% ============================================================
% EXPERIMENT STATUS (headline + secondary arms filled)
% ============================================================
%
% DONE   E7/E10 secondary ablations (protocol-matched, 170 ep)
%            disc3d 26.05 / disc4d 25.97   (stacked disc dropped)
%            perc0.5 26.46 / perc3.0 25.66 / kl1e-7 26.05   (vs E1d 25.31)
%            rank8 31.23 / rank128 32.13                   (vs E1c 31.21)
%            lora_before TC=on 25.98 / TC=off 31.47
%
...
\label{eq:objective}
\end{equation}
The KL term is always included, at weight $10^{-6}$, the standard tokenizer setting \cite{rombach2022high}, and is logged separately during training: the latent must stay a proper VAE posterior for downstream diffusion, not degrade into a plain autoencoder. An optional adversarial term uses a PatchGAN-style 3D discriminator \cite{isola2017image}: either views flattened into the batch, or an explicit 4D discriminator with a view axis and per-view embeddings.
...
The default protocol uses no discriminator, perceptual weight $1.5$, and KL weight $10^{-6}$. Re-running E1d with the 3D PatchGAN (views flattened) and the joint 4D discriminator, and with lower/higher perceptual or KL weight (Tab.~\ref{tab:loss}), shows that none of these choices closes the joint-compression gap. Both discriminators improve PSNR by ${+}0.66$--${+}0.74$~dB over the no-disc baseline, with a matching LPIPS drop (0.082$\to$0.069); they are nearly equivalent to each other. Lowering the perceptual weight to $0.5$ is the best loss tweak (${+}1.15$~dB), while raising it to $3.0$ helps less (${+}0.35$~dB). Lowering KL to $10^{-7}$ matches the 3D disc gain (${+}0.74$~dB). These gains are small relative to width (${+}2.40$~dB) or the temporal-difference loss (${+}1.83$~dB), and none removes bleeding.

\begin{table}[t]
\centering
\caption{Loss configuration under joint compression (fused, TC on). Default row is E1d.}
\label{tab:loss}
\begin{tabular}{lrrr}
\toprule
...
\midrule
3D PatchGAN, views flattened            & 26.05 & 0.069 & 0.903 \\
joint 4D discriminator                  & 25.97 & 0.069 & 0.910 \\
\midrule
perceptual weight $0.5$                 & \textbf{26.46} & 0.071 & 0.882 \\
perceptual weight $3.0$                 & 25.66 & 0.069 & 0.901 \\
KL weight $10^{-7}$                     & 26.05 & 0.069 & 0.907 \\
\bottomrule
\end{tabular}
\end{table}
...
We sweep rank $\{8,32,128\}$ at the E1c setting (fused, TC off), holding data and budget fixed (Tab.~\ref{tab:lora_rank}). Rank 8 matches the default rank 32 (31.23 vs.\ 31.21~dB). Rank 128 gains ${+}0.92$~dB (32.13~dB) and improves LPIPS (0.044$\to$0.033). The plateau starts by rank 8; larger ranks help a little but are not required for the main claims, which use rank 32.

\begin{table}[t]
\centering
\caption{LoRA rank at fused TC=off. Rank 32 is E1c.}
\label{tab:lora_rank}
\begin{tabular}{lrrr}
\toprule
...
128          & \textbf{32.13} & \textbf{0.033} & 0.979 \\
\bottomrule
\end{tabular}
\end{table}
...
The default places LoRA only after fusion (bottleneck and decoder). Adding LoRA on the frozen per-view encoder stem ($E_{\mathrm{pre}}$) tests whether the joint-compression failure is an under-adapted encoder or a bottleneck after fusion (Tab.~\ref{tab:lora_before}). Under TC off, before+after gains ${+}0.26$~dB (31.47 vs.\ 31.21). Under TC on --- the joint-compression setting --- the gain is ${+}0.67$~dB (25.98 vs.\ 25.31), still far from closing the gap to the per-view TC reference (31.50~dB). Adapting the stem helps a little; the information is still missing from the latent.

\begin{table}[t]
\centering
\caption{Pre-fusion encoder LoRA. Default is after-only.}
...
off & before + after       & 31.47 & 0.038 & 0.972 \\
\midrule
on  & after only (E1d)     & 25.31 & 0.082 & 0.919 \\
on  & before + after       & 25.98 & 0.069 & 0.903 \\
\bottomrule
\end{tabular}
\end{table}
