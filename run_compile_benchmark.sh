...
# torch.compile benchmark — one SLURM array task per mode, all run in parallel.
#
# Submit:   sbatch run_compile_benchmark.sh
# Monitor:  squeue -u $USER
# Results:  outputs/compile_benchmark/summary.json  (written by each task; last one prints table)
#
# Modes (5 tasks):
#   1  no_compile                  — baseline, torch.compile disabled
#   2  compile_default             — mode="default"            (current config)
#   3  compile_reduce_overhead     — mode="reduce-overhead"    (CUDA graphs)
#   4  compile_max_autotune        — mode="max-autotune"       (full tuning + CUDA graphs)
#   5  compile_max_autotune_no_cg  — mode="max-autotune-no-cudagraphs"
#
# Metrics per run:
#   compile_load_time_s            — wall time until Step 1 log line (load + first-step compile)
#   step10_avg_time_s              — avg of first 10 steps (from [step_time] log message)
#   step50_total_step_s            — time/total_step at Step 50 (clean post-warmup speed)
#   gpu_util_warm_{avg,min,max}_pct — GPU SM utilisation during Steps 40–60
...
RESULTS_DIR="${RESULTS_DIR:-${OPEN_SORA_ROOT}/outputs/compile_benchmark}"
...
# ── Benchmark dataset / step budget ───────────────────────────────────────────
# single_sequence: exactly 1 .pt file → 1 gradient step per epoch.
# accumulation_steps=1 → each gradient step == 1 optimizer update.
# 70 epochs → 70 update steps; Step 50 is safely past warmup.
BENCH_EPOCHS=70

# ── Experiment table (1-indexed to match SLURM_ARRAY_TASK_ID) ─────────────────
# Format: "display_name|extra_cli_overrides"
EXPERIMENTS=(
...
# gpu_poller.py — samples nvidia-smi every second, writes elapsed_s,gpu%,mem% CSV
...
# timestamper.py — prepends [T+XX.XXXs] to every stdin line (used in the pipe)
...
# parse_run.py — extracts metrics from one run and writes <name>_result.json
...
# If all result files are present, print summary table
import glob, os
expected = [
    "no_compile",
    "compile_default",
...
# ── Per-task banner ────────────────────────────────────────────────────────────
echo "════════════════════════════════════════════════════════════════════"
echo "  torch.compile benchmark — task ${SLURM_ARRAY_TASK_ID:-?}/${n_exp}: ${exp_name}"
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
...
echo "    --data_preset single_sequence  --batch_size 1  --accumulation_steps 1"
echo "    --epochs ${BENCH_EPOCHS}  --log_every 1  --log_bottleneck_every 1"
echo "    --wandb False  --save_ckpt False  --eval_every 999999"
echo "    --final_eval False  --profile_step False  --deterministic True"
echo "  Log  → ${log_file}"
echo "  GPU  → ${gpu_file}"
echo ""
...
accelerate launch \
  ...
  "$train_file" \
  "$my_config" \
  --experiment_name "benchmark_${exp_name}" \
  --wandb_expr_name "benchmark_${exp_name}" \
