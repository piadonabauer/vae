...
# 128px, T=5, TC=True, no disc, 2-view — generalization (all_people_one_expression).
#
# Submit from /project (not /home):
#   cd /project/6101839/piado/vae
#   sbatch --job-name=tc_2v128_t5_all --time=0-15:00:00 --export=ALL,TASK=2 ./run_tc_2view_128_t5_gen.sh
#
# Optional one-person warm-up (TASK=1, 5h):
#   sbatch --job-name=tc_2v128_t5_1p --time=0-05:00:00 --export=ALL,TASK=1 ./run_tc_2view_128_t5_gen.sh
#
# Bucket: 128px × 5 frames  (on-disk T=13 → linspace indices [0,3,6,9,12])
# Latent with TC: T' = 1+(5-1)//4 = 2  (chunks 1|4 → decode 1+4=5)
...
# Optional: warm-start from a prior TC run (leave empty to start from Wan pretrained only).
LOAD_CKPT="${LOAD_CKPT:-}"

if [[ "${1:-}" == "submit" ]]; then
  mkdir -p "${OPEN_SORA_ROOT}/slurm_logs"
  j1=$(sbatch --parsable --job-name=tc_2v128_t5_1p  --time=0-05:00:00 \
    --export=ALL,TASK=1 "$SCRIPT_PATH")
  j2=$(sbatch --parsable --job-name=tc_2v128_t5_all --time=0-15:00:00 \
    --export=ALL,TASK=2 "$SCRIPT_PATH")
  echo "Submitted TASK=1 (one_person / single_seq, 5h): job ${j1}"
  echo "Submitted TASK=2 (all_people_one_expression, 15h): job ${j2}"
  exit 0
fi
...
case "$TASK" in
  1)
    # one person / single clip warm-up
    wandb_name="tc_true_2v128_t5_single_seq"
    ...
    ;;
  2)
    wandb_name="tc_true_2v128_t5_all_people_one_expr"
    ...
    # T=5 is lighter than T=9; still start conservative after prior 32 OOM at T=9
...
if [[ -n "$LOAD_CKPT" ]]; then
  EXTRA_OVERRIDES+=( --load "$LOAD_CKPT" )
fi
...
echo "════════════════════════════════════════════════════════════════════"
echo "  tc_2v128_t5 TASK=${TASK} — ${wandb_name}"
echo "  data_preset : ${data_preset}"
echo "  bucket      : ${BUCKET_CONFIG}"
echo "  load        : ${LOAD_CKPT:-<none>}"
