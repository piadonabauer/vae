...
# Train on 8-view preprocessed data with automatic batch-size selection.
#
# Resolution is set via RESOLUTION env var (default 512):
#   sbatch --export=RESOLUTION=256  run_8v_allpeople_autobatch.sh
#   sbatch --export=RESOLUTION=512  run_8v_allpeople_autobatch.sh
#   sbatch --export=RESOLUTION=1024 run_8v_allpeople_autobatch.sh
#   sbatch --export=RESOLUTION=2048 run_8v_allpeople_autobatch.sh
#
# Batch-size probe:
#   Tries batch sizes 32 → 16 → 8 → 4 → 2 → 1 (each for 5 optimizer steps).
...
exp_name="8v_${RESOLUTION}px_allpeople_one_expr_lr5e4_compile"
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
...
if [[ -n "${SLURM_JOB_ID:-}" ]]; then
  scontrol update jobid="${SLURM_JOB_ID}" name="8v_${RESOLUTION}px_autobatch" 2>/dev/null || true
fi

echo "=== 8-view ${RESOLUTION}px all_people_one_expression (auto batch) ==="
...
common_args=(
  ...
  --wandb_expr_name  "8v_${RESOLUTION}px_allpeople_one_expr_lr5e4_compile"
  --nersemble_processed_base /datasets/lindell-proj/neumayr/nersemble_v2/processed/8-views
...
if [[ -z "$selected_batch" ]]; then
  echo "ERROR: All batch sizes OOM'd. Cannot train at ${RESOLUTION}px with 8 views on this GPU."
...
echo ""
echo "=== Full training: batch=${selected_batch}  accum=${selected_accum}  res=${RESOLUTION}px  8v ==="
